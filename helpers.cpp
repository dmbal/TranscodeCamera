...
void DetectSubtype(GUID guid, DebugInfoStorage* debugInfo) {
    ...
    if (my_map.count(guid) != 0) {
        ...
        debugInfo->AppendString(str.c_str());
    }
    else {
        debugInfo->AppendGuid(L"undefined subtype: ", guid);
    }
...
IMFTransform* FindEncoderTransform(GUID out_video_format, DebugInfoStorage* debugInfo) {
    UINT32 count = 0;
    IMFActivate **ppActivate = NULL;
    DetectSubtype(out_video_format, debugInfo);
...
void DebugInfoWithLevel(std::wstring pref, int level, DebugInfoStorage* debugInfo) {
    debugInfo->AppendString(L"\\");
    for (int i = 0; i < level; i++)
    {
        debugInfo->AppendString(L"--");
    }
    debugInfo->AppendString(pref);
}

HRESULT HandleChildren(IMFTopologyNode * node, int level, DebugInfoStorage* debugInfo) {
    DWORD outputs = 0;
    ...
    for (DWORD i = 0; i < outputs; i++) {
        ...
        if (SUCCEEDED(hr))
        {
            UnwrapPartialTopo(childNode, level, debugInfo);
        }
    }
    return hr;
}

HRESULT HandleNodeObject(IMFTopologyNode * node, DebugInfoStorage* debugInfo) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (activate != NULL) {
            ...
            debugInfo->AppendString(L"activate");
        }
        ...
        if (transform != NULL) {
            ...
            if (SUCCEEDED(hr)) {
                debugInfo->AppendString(L"input type: ");
                debugInfo->AppendString(DetectMajorType(GetMajorType(inputType)));
                debugInfo->AppendString(L" ");
                DetectSubtype(GetVideoSubtype(inputType), debugInfo);
                debugInfo->AppendString(L"; ");
            }
            CComPtr<IMFMediaType> outputType;
            hr = transform->GetOutputCurrentType(0, &outputType);
            if (SUCCEEDED(hr)) {
                debugInfo->AppendString(L"output type: ");
                debugInfo->AppendString(DetectMajorType(GetMajorType(outputType)));
                debugInfo->AppendString(L" ");
                DetectSubtype(GetVideoSubtype(outputType), debugInfo);
            }
...
HRESULT HandleSourceNode(IMFTopologyNode * node, DebugInfoStorage* debugInfo) {
    DebugInfoWithLevel(L"[SOURCE]", 0, debugInfo);
    HandleNodeObject(node, debugInfo);
    debugInfo->AppendString(L"\n");
    HRESULT hr = HandleChildren(node, 1, debugInfo);
    return hr;
}


HRESULT HandleTeeNode(IMFTopologyNode * node, int level, DebugInfoStorage* debugInfo) {
    DebugInfoWithLevel(L"[TEE]", level, debugInfo);
    HandleNodeObject(node, debugInfo);
    debugInfo->AppendString(L"\n");
    HRESULT hr = HandleChildren(node, level + 1, debugInfo);
    return hr;
}

HRESULT HandleTransformNode(IMFTopologyNode * node, int level, DebugInfoStorage* debugInfo) {
    DebugInfoWithLevel(L"[TRANSFORM]", level, debugInfo);
    HandleNodeObject(node, debugInfo);
    debugInfo->AppendString(L"\n");
    HRESULT hr = HandleChildren(node, level + 1, debugInfo);
    return hr;
...
HRESULT HandleSinkNode(IMFTopologyNode * node, int level, DebugInfoStorage* debugInfo) {
    DebugInfoWithLevel(L"[SINK]", level, debugInfo);
    HandleNodeObject(node, debugInfo);
    debugInfo->AppendString(L"\n");
    HRESULT hr = HandleChildren(node, level + 1, debugInfo);
    return hr;
...
HRESULT UnwrapPartialTopo(IMFTopologyNode * node, int level, DebugInfoStorage* debugInfo) {
    ...
    if (type == MF_TOPOLOGY_SOURCESTREAM_NODE) {
        HandleSourceNode(node, debugInfo);
    }
    if (type == MF_TOPOLOGY_TEE_NODE) {
        HandleTeeNode(node, level, debugInfo);
    }
    if (type == MF_TOPOLOGY_OUTPUT_NODE) {
        HandleSinkNode(node, level, debugInfo);
    }
    if (type == MF_TOPOLOGY_TRANSFORM_NODE) {
        HandleTransformNode(node, level, debugInfo);
    }
    return hr;
}
...
HRESULT UnwrapTopo(IMFTopology * pTopology) {
    WORD nodeCount = 0;
    std::wstringstream strstream;
    DebugInfoStorage* debugInfo = new DebugInfoStorage();
    debugInfo->AppendString(L"\n");
    debugInfo->AppendString(L"======================\n");
    debugInfo->AppendString(L"DEBUG TOPOLOGY\n");
    ...
    for (WORD i = 0; i < nodeCount; i++) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
                if (type == MF_TOPOLOGY_SOURCESTREAM_NODE) {
                    hr = UnwrapPartialTopo(node, 0, debugInfo);
                }
            }
        }
    }
    debugInfo->AppendString(L"======================\n");
    debugInfo->PrintResult();
    return hr;
}